...
Incoming files are stored here before validation and movement to the D1 Staged/Enforced layer in BigQuery.
...
Keeping raw and validated data separate helps prevent bad input and schema mismatches from reaching the trusted data layer.
...
This allows the ingestion workload to create objects without giving it unnecessary permissions such as deleting objects or managing the bucket.
...
Terraform also manages the required APIs and service accounts used by the architecture.
...
The bucket itself has not been created because the GCP project currently has no active billing account.

Therefore, the project does not claim that the bucket is deployed. The Terraform configuration is ready to apply when billing is available.
